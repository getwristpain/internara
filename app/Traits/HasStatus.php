...
trait HasStatus
{
     ...
     */
    public function firstStatus(string $name = '', string $type = ''): ?Status
    {
        $conditions = Helper::filter(['type' => $type, 'name' => $name]);
    ...
    {
        $status = $this->firstStatus($name, $type);

        if (! $status) {
            return false;
        }

        return $status->update([$column => ($strict ?: !$status->{$column})]);
    }
