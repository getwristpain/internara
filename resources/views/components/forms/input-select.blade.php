...
<div class="flex flex-col gap-2 w-full font-medium pt-1 {{ $disabled ? 'disabled' : '' }}">
    <div class="flex flex-col w-full gap-2">
        <x-forms.input-label :name="$name" :label="$label" :required="$required"
            :hint="$hint"></x-forms.input-label>
        ...
        <div class="relative w-full" x-data="{
            ...
            <div class="flex w-full gap-2 items-center justify-between cursor-pointer input input-bordered focus:ring-2 focus:ring-neutral {{ $disabled ? 'opacity-80 cursor-not-allowed' : '' }}"
                @click="if (!{{ $disabled ? 'true' : 'false' }}) { open = !open }" tabindex="0">
            ...
            <div class="absolute z-50 w-full mt-1 bg-white border rounded-md" x-show="open" @click.away="open = false"
                ...
                <template x-if="showSearch">
                    <input class="w-full p-2 border rounded-t-md" type="text" x-model="search"
                        ...
                        {{ $disabled ? 'disabled' : '' }} {{ $required ? 'required' : '' }} autofocus tabindex="0" />
                </template>
