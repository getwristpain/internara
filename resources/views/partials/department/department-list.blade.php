...
<div class="wh-full flex flex-col items-center justify-center gap-12">
    <x-form class="w-full p-4" :$submit name="departmentForm" bordered>
        <div class="flex justify-center items-center gap-4 flex-grow">
            <x-input type="text" field="form.data.name" placeholder="Masukkan nama jurusan..." required autofocus />
            <x-button class="max-sm:btn-circle flex-row-reverse" labelClass="hidden sm:inline" label="Tambah"
                type="submit" icon="mdi:plus" form="departmentForm" title="Tambah Jurusan" />
        </div>
    </x-form>
    ...
    <div class="flex-1 w-full max-w-2xl flex flex-wrap gap-4 justify-center" x-data="{
        ...
        <template x-if="departments.lenght > 0">
